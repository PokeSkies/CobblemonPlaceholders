            ...
            .audiencePlaceholder("party_aspects") { audience, queue, _ ->
                ...
                if (queue.peek() != null) {
                    ...
                    if (aspectSlot.isEmpty)
                        return@audiencePlaceholder Tag.inserting(Component.text("Invalid aspect slot argument (1+)!"))

                    return@audiencePlaceholder Tag.inserting(Component.text(
                        if (aspectSlot.asInt <= pokemon.aspects.size) pokemon.aspects.toList()[aspectSlot.asInt - 1] else "Empty"
                    ))
                }
