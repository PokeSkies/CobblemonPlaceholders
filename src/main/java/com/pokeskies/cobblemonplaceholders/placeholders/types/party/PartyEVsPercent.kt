...
class PartyEVsPercent : CobblemonPlaceholder {
    override fun register(builder: Expansion.Builder) {
        builder.filter(ServerPlayerEntity::class.java)
            .audiencePlaceholder("party_evs_percent") { audience, queue, _ ->
                if (queue.peek() == null)
                ...
                val sum = Stats.values().toList().stream().mapToInt { pokemon.evs[it] ?: 0 }.sum()
                ...
                return@audiencePlaceholder Tag.inserting(Component.text(
                    Utils.parseDouble((EVs.MAX_TOTAL_VALUE.toDouble() / sum) * 100)
                ))
            }
    }
}