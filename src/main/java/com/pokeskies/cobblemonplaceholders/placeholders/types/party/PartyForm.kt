    ...
    override fun register(builder: Expansion.Builder) {
        builder.filter(ServerPlayer::class.java)
            .audiencePlaceholder("party_form") { audience, queue, _ ->
                ...
                return@audiencePlaceholder Tag.inserting(Component.text(Utils.titleCase(pokemon.form.name)))
            }
    }
}
