...
class PartyFriendship : CobblemonPlaceholder {
    override fun register(builder: Expansion.Builder) {
        builder.filter(ServerPlayerEntity::class.java)
            .audiencePlaceholder("party_friendship") { audience, queue, _ ->
                if (queue.peek() == null)
                ...
                return@audiencePlaceholder Tag.inserting(Component.text(pokemon.friendship))
            }
    }
}