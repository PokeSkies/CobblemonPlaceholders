...
class PartyHeldItem : CobblemonPlaceholder {
    override fun register(builder: Expansion.Builder) {
        builder.filter(ServerPlayer::class.java)
            .audiencePlaceholder("party_helditem") { audience, queue, _ ->
                if (queue.peek() == null)
                ...
                return@audiencePlaceholder Tag.inserting(Component.text(pokemon.heldItem().displayName.string))
            }
    }
}
