    ...
    override fun register(builder: Expansion.Builder) {
        builder.filter(ServerPlayerEntity::class.java)
            .audiencePlaceholder("party_nature") { audience, queue, _ ->
                ...
                return@audiencePlaceholder Tag.inserting(Component.text(Utils.titleCase(pokemon.nature.name.path)))
            }
    }
}