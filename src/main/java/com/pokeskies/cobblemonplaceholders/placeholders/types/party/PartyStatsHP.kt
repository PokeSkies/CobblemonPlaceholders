...
class PartyStatsHP : CobblemonPlaceholder {
    override fun register(builder: Expansion.Builder) {
        builder.filter(ServerPlayer::class.java)
            .audiencePlaceholder("party_stats_hp") { audience, queue, _ ->
                if (queue.peek() == null)
                ...
                return@audiencePlaceholder Tag.inserting(Component.text(pokemon.hp))
            }
    }
}
