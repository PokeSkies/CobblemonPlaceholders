...
class PartyStatsSpecialAttack : CobblemonPlaceholder {
    override fun register(builder: Expansion.Builder) {
        builder.filter(ServerPlayerEntity::class.java)
            .audiencePlaceholder("party_stats_spa") { audience, queue, _ ->
                if (queue.peek() == null)
                ...
                return@audiencePlaceholder Tag.inserting(Component.text(pokemon.specialAttack))
            }
    }
}