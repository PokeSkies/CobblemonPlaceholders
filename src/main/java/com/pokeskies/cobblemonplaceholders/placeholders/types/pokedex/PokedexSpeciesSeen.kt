...
class PokedexSpeciesSeen : PlayerPlaceholder {
    override fun handle(player: ServerPlayer, args: List<String>): GenericResult {
        if (args.isEmpty()) return GenericResult.invalid(Component.text("No species provided!"))
        ...
        return GenericResult.valid(Component.text(manager.getKnowledgeForSpecies(species.resourceIdentifier) != PokedexEntryProgress.NONE))
    }

    override fun id(): String = "pokedex_species_seen"
}
